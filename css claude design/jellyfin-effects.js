...
(function () {
  ...
  var MAX_TILT = 9;          // degrés d'inclinaison max (intensité élevée)
  var LIFT     = 14;         // élévation en px au survol
  ...
  function initTilt() {
    ...
    function apply() {
      ...
      current.style.transform =
        ...
        rotY.toFixed(2) + "deg) translateY(-" + LIFT + "px) scale(1.07)";
    }
  ...
  function initReveal() {
    ...
    function scan(root) {
      (root.querySelectorAll ? root.querySelectorAll(".card, .verticalSection") : [])
        .forEach(watch);
    }
    scan(document);
    ...
    new MutationObserver(function (muts) {
      muts.forEach(function (m) {
        m.addedNodes.forEach(function (n) {
          ...
          if (n.matches && n.matches(".card, .verticalSection")) watch(n);
          scan(n);
        });
      });
